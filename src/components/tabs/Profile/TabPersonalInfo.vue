    ...
    <form class="inner_tab row" @submit.prevent="someAction()">
        ...
        <div>
          ...
          <label :class="{ row : true , error1 : $v.tab_personal_birthDate.$invalid}">
            ...
            <input v-validate data-vv-as="дата рождения" v-model="tab_personal_birthDate" class="form__input col-sm"
                   type="date" name="birthday" min="1918-01-01" max="2019-01-01"/>
          </label>
      ...
      <div class="info_pasp col-sm">
        ...
        <label class="row">
          ...
          <input v-if="tab_personal_isCompatriot==true" v-model="tab_personal_isEquatedForeign" class="checkbox col-sm"
                 type="checkbox" disabled>
          <input v-else v-model="tab_personal_isEquatedForeign" class="checkbox col-sm" type="checkbox"
                 id="equate_foreign">
        </label>
